...
1. **Clean-room.** The v0 prototype is historical motivation only. Do not read, copy, port, or use its code, tests, schemas,
...
```bash
# Prefer the venue-provided anonymous source snapshot.  If the venue supplies
# an actual anonymous Git remote, cloning is also supported:
if [ -n "${ANONYMOUS_REPOSITORY_URL:-}" ]; then
  git clone "$ANONYMOUS_REPOSITORY_URL" GEML
  cd GEML
else
  GEML_SOURCE_DIR=/absolute/path/to/GEML-anonymous-source
  test -d "$GEML_SOURCE_DIR"
  cd "$GEML_SOURCE_DIR"
fi
...
Tests must run on a fresh source copy with **no** `outputs/` directory and **no** production
...
Reproducibility instructions live here and nowhere else. (Phase-A package for issues
12-1 and 12-2, prepared 2026-07-26.)
...
Use Python 3.12 or newer. Obtain the anonymous source snapshot supplied with the
submission and record its archive or directory SHA-256 before running commands. Do
not invent a remote URL or reproduce from a moving branch name. If the venue supplies
an actual anonymous Git remote and an exact snapshot ref, use the optional clone path
below; otherwise operate directly on the downloaded source directory.
...
set -euo pipefail
if [ -n "${ANONYMOUS_REPOSITORY_URL:-}" ]; then
  git clone "$ANONYMOUS_REPOSITORY_URL" GEML-release
  cd GEML-release
  if [ -n "${ANONYMOUS_SNAPSHOT_REF:-}" ]; then
    git fetch --tags --force
    git checkout --detach "$ANONYMOUS_SNAPSHOT_REF"
    test "$(git rev-parse HEAD)" = "$ANONYMOUS_SNAPSHOT_REF"
  fi
else
  GEML_SOURCE_DIR=/absolute/path/to/GEML-anonymous-source
  test -d "$GEML_SOURCE_DIR"
...
if ($env:ANONYMOUS_REPOSITORY_URL) {
    git clone $env:ANONYMOUS_REPOSITORY_URL GEML-release
    Set-Location GEML-release
    if ($env:ANONYMOUS_SNAPSHOT_REF) {
        git fetch --tags --force
        git checkout --detach $env:ANONYMOUS_SNAPSHOT_REF
        if ((git rev-parse HEAD) -ne $env:ANONYMOUS_SNAPSHOT_REF) {
            throw "Anonymous snapshot reference mismatch"
        }
    }
} else {
    $GEML_SOURCE_DIR = 'C:\path\to\GEML-anonymous-source'
    if (-not (Test-Path -LiteralPath $GEML_SOURCE_DIR -PathType Container)) {
        throw "Set GEML_SOURCE_DIR to the downloaded anonymous source snapshot"
    }
    Set-Location -LiteralPath $GEML_SOURCE_DIR
}
...
Record the anonymous source snapshot reference or directory digest, lock SHA-256, Python version, package inventory, operating system,
CPU, RAM, storage, GPU model/driver, CUDA version, deterministic settings, and precision in
the final run envelope.

### Anonymous Goals 1-5 artifact handoff

Private cloud URLs and provider-specific IDs are intentionally omitted from this double-blind
snapshot. Obtain the artifact archive, checksum file, extraction scripts, and index from the
anonymous supplementary-artifact host named by the submission. Authenticate every download
before executing a script or trusting an index. The archive is an uncompressed TAR, not a ZIP.
Provision at least 80 GB free space and at least 1.3 million free inodes. Extract outside the
Git checkout; when hosts share the archive, make the extracted tree read-only and share only its
absolute path. The checked-in `scripts/repro/ARTIFACT_SOURCES.json` retains content hashes and
file sizes but deliberately contains no private-storage URL or account identifier.
...
fresh-source audit must still verify that no smoke opens a network connection. Each uses only
...
every cell to its config content/hash, anonymous source snapshot,
...
3. Recreate the same anonymous source snapshot and locked environment.
...
- records the anonymous source snapshot reference or directory digest, Python/platform/package inventory, profile, seeds, and config
  checksums;
...
1. Create a new temporary source copy from the downloaded anonymous snapshot. If and only if
   an actual anonymous Git remote and exact snapshot reference are supplied, clone it and
   detach at that reference.
