...
def load_artifact_manifest(path: Path = ARTIFACT_MANIFEST_PATH) -> dict[str, Any]:
    ...
    if not isinstance(public_url, str) or not public_url.startswith(
        ...
        raise ValueError("artifact registry must contain a public Google Drive folder URL")
    ...
    for item in files:
        ...
        if not isinstance(drive_id, str) or not re.fullmatch(r"[A-Za-z0-9_-]+", drive_id):
            raise ValueError(f"missing Drive ID for {name}")
        if drive_id in drive_ids:
            raise ValueError(f"duplicate Drive ID for {name}")
        names.add(name)
