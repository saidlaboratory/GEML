...
def test_anonymous_artifact_registry_is_complete_and_authenticated() -> None:
    payload = load_artifact_manifest()

    assert payload["public_folder_url"] is None
...
def test_artifact_registry_rejects_code_host_for_large_artifacts(tmp_path: Path) -> None:
    payload = json.loads(ARTIFACT_MANIFEST_PATH.read_text(encoding="utf-8"))
    payload["public_folder_url"] = "https://anonymous.4open.science/anonymous-code"
    path = tmp_path / "code-host.json"
    path.write_text(json.dumps(payload), encoding="utf-8")

    with pytest.raises(ValueError, match="HTTPS OSF URL or null"):
        load_artifact_manifest(path)

    payload["public_folder_url"] = "https://osf.io/anonymous-artifacts"
    path.write_text(json.dumps(payload), encoding="utf-8")
    assert load_artifact_manifest(path)["public_folder_url"] == (
        "https://osf.io/anonymous-artifacts"
    )
