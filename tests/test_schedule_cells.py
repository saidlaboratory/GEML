...
def _timeout_cmd() -> str:
    """Build a shell command that sleeps on both POSIX shells and Windows cmd."""

    if os.name == "nt":
        # cmd.exe does not provide the POSIX ``sleep`` utility or parameter
        # expansion.  Keep the shell token as a literal Python argument so
        # render_command still proves it leaves non-cell braces untouched.
        python_code = (
            "import sys,time; "
            "print(sys.argv[1].replace('${SCHED_UNSET:-tick}', 'tick'), flush=True); "
            "time.sleep(60)"
        )
        return f'"{sys.executable}" -c "{python_code}" "${{SCHED_UNSET:-tick}} {{cell_id}}"'
    return 'echo "${SCHED_UNSET:-tick} {cell_id}" && sleep 60'
