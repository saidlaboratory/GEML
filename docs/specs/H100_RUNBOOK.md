...
Status: prepared 2026-07-30 against the anonymous release snapshot.
Companion tooling: `scripts/h100/bootstrap.sh`,
...
1. **The source snapshot must be available.** The anonymous release snapshot
   is supplied with the submission and incorporates the reviewed workstreams on top of
   the anonymous paper snapshot `ANON-PAPER-SNAPSHOT`:
   - the anonymous pairs-build snapshot `ANON-G6-PAIRS-SNAPSHOT` — the real production
     pair-build loop behind `python -m geml.data.pairs`;
   - anonymous paper snapshot `ANON-PAPER-SNAPSHOT`;
   - anonymous symbolic-regression snapshot `ANON-SR-SNAPSHOT`.
   Publish or download the anonymous release snapshot so the node can use
   it together with the scheduler tooling included in this snapshot.
   `bootstrap.sh` refuses to guess: it takes an explicit source reference.
...
5. **Goal 6 width freeze.** `configs/goal6_grid.yaml` has
   ...
   preflight tool is supplied as the companion width-preflight snapshot;
...
```bash
GEML_REPO_URL='<anonymous-Git-remote-if-supplied>' \
GEML_COMMIT=<supplied-anonymous-snapshot-ref> \
bash bootstrap.sh          # or scripts/h100/bootstrap.sh from the source snapshot
```
...
| d | `width-preflight` | BLOCKED (prereq 5) — companion width-preflight snapshot | ? | ? | pending |
...
**(d) Width preflight.** Reference only — supplied in the companion width-preflight snapshot,
