...
# Panel A: regression error looks fine
ax1.bar(x - w/2, val_mae, w, color=c_err, label="val")
ax1.bar(x + w/2, iid_mae, w, color=c_err, alpha=0.5, label="test-IID")
ax1.set_title("Regression error (MAE)", fontsize=8)
ax1.set_xticks(x); ax1.set_xticklabels(seeds); ax1.set_xlabel("seed")
ax1.set_ylim(0, 2.1); ax1.legend(frameon=False, fontsize=6, loc="upper left")
